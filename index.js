...
const recoveredItemsCollection = client.db('WhereIsIt').collection('recoveredItems');
...
app.post('/recoveredItems', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).send({ success: false, message: 'Failed to save recovered item' });
  }
});
...
app.patch('/items/:id', async (req, res) => {
  try {
    ...
    if (!updatedStatus) {
      return res.status(400).send({ success: false, message: 'Missing status in request body' });
    }
...
app.get('/recoveredItems', async (req, res) => {
  try {
    ...
    console.error(error);
    res.status(500).send({ success: false, message: 'Failed to fetch recovered items' });
  }
});
